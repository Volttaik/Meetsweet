...
- `artifacts/meetsweet` — Expo React Native mobile app (connects only to new backend)
- Old backend (`artifacts/api-server`) is deprecated — remove old integrations from mobile app
...
- `getApiBase()` in `artifacts/meetsweet/services/api.ts` appends `/api` automatically
