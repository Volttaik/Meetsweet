...
import { buildShortRow, groupMediaByPost, visibleContentCondition } from "@/lib/services/content";
...
export async function GET(
  ...
  const shorts = items.map((p) =>
    buildShortRow(p, mediaByPost[p.id] ?? [], likedSet.has(p.id), isSubscribed, subTier, isOwner),
  );
