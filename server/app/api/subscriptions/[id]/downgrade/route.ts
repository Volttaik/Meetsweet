...
export async function POST(
  ...
  if (newPrice >= currentPrice) {
    return err("Downgrade tier must be lower than current tier", 400);
  }

  // Update the subscription tier (takes effect at end of billing period in real implementation)
  await db
  ...
  return ok({ subscription: updated, effective_at: sub.expires_at });
}
