...
export async function POST(
  ...
  const [post] = await db.select({ id: posts.id, like_count: posts.like_count })
    .from(posts).where(and(eq(posts.id, id), eq(posts.content_type, "short"))).limit(1);
  if (!post) return err("Short not found", 404);

  const [existing] = await db.select({ id: post_likes.id }).from(post_likes)
    .where(and(eq(post_likes.user_id, auth.user.userId), eq(post_likes.post_id, id))).limit(1);
...
export async function DELETE(
  ...
  const [post] = await db.select({ id: posts.id }).from(posts)
    .where(and(eq(posts.id, id), eq(posts.content_type, "short"))).limit(1);
  if (!post) return err("Short not found", 404);
