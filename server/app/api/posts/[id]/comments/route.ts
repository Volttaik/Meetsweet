...
import { users, profiles, posts, comments, comment_likes, notifications } from "@/lib/db/schema";
...
export async function POST(
  ...
  // Notify post creator (skip if creator commented on their own post)
  if (post.creator_id && post.creator_id !== auth.user.userId) {
    ...
    const preview = parsed.data.body.length > 60
      ? parsed.data.body.slice(0, 57) + "…"
      : parsed.data.body;
